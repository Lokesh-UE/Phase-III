...
async function checkApiHealth() {
  ...
  if (!API_BASE) {
    ...
    pill.title = "Using same-origin API (Flask local server)";
    return;
  }
  ...
  try {
    ...
    const data = await response.json();
    pill.textContent = data.model_ready ? "Model ready" : "Model missing";
    pill.className = `status-pill ${data.model_ready ? "ready" : "missing"}`;
    if (alert) hide(alert);
  } catch {
